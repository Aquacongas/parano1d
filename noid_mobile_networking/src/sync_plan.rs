...
/// Exact canonical progress may retire a pending snapshot only when the
/// durable canonical chain actually contains the snapshot's exact boundary.
///
/// Height alone is insufficient: a competing branch may already have advanced
/// beyond the snapshot height while carrying a different block at that height.
pub fn canonical_tip_supersedes_snapshot_boundary(
    canonical_tip_height: u64,
    canonical_boundary: Option<crate::ChainPoint>,
    snapshot_boundary: crate::ChainPoint,
) -> bool {
...
#[cfg(test)]
mod canonical_snapshot_boundary_tests {
    use super::canonical_tip_supersedes_snapshot_boundary;

    #[test]
    fn exact_progress_retires_only_matching_snapshot_boundary() {
        let snapshot = crate::ChainPoint::new(54, [1; 32]);
        let competing = crate::ChainPoint::new(54, [2; 32]);
