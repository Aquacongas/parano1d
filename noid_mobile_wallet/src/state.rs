...
pub struct WalletState {
    ...
    pub next_index: u32,
    /// In-memory lookup for every generated public address.
    ///
    /// This contains public owner bytes only, never spend secrets.  Keeping it
    /// cached avoids deriving the whole generated prefix for every accepted
    /// block when wallet-wide RECEIVED history is updated.
    pub(super) generated_address_index: HashMap<[u8; 32], u32>,
...
impl WalletState {
    ...
    pub fn mark_receipts_dirty(&mut self) {
    ...
    pub fn receipts_dirty(&self) -> bool {
        self.receipts_dirty
    }
    ...
    pub fn create_or_load(key_path: PathBuf) -> Result<Self, KeystoreError> {
        ...
        let mut wallet = Self {
            ...
            utxos: HashMap::new(),
            next_index: 1,
            generated_address_index: HashMap::new(),
        ...
        wallet.rebuild_generated_address_index();
        wallet.load_receipts().map_err(KeystoreError::Artifact)?;
        wallet.load_history().map_err(KeystoreError::Artifact)?;
        Ok(wallet)
    }

    fn rebuild_generated_address_index(&mut self) {
        self.generated_address_index.clear();
        self.generated_address_index
            .reserve(self.next_index as usize);

        for index in 0..self.next_index {
            let address = self.secret.derive_address(index);
            self.generated_address_index.insert(address.0, index);
        }
    }

    /// Ensure the public-address lookup covers exactly the generated prefix.
    ///
    /// The length check also self-heals tests/development code that mutates
    /// `next_index` directly instead of going through the normal metadata
    /// commit helpers.
    pub(super) fn ensure_generated_address_index(&mut self) {
        if self.generated_address_index.len() != self.next_index as usize {
            self.rebuild_generated_address_index();
            return;
        }

        let active = self.secret.derive_address(self.active_index);
        if self.generated_address_index.get(&active.0).copied() != Some(self.active_index) {
            self.rebuild_generated_address_index();
        }
    }
    ...
    pub fn commit_discovered_next_index(
        ...
        for index in self.next_index..discovered_next_index {
            let address = self.secret.derive_address(index);
            self.generated_address_index.insert(address.0, index);
        }
