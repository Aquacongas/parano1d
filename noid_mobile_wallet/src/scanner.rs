...
//! bodies while they are available.
//!
//! ## Incremental update (new block)
//!
//! Active UTXO mutation remains strictly one-owner-only.  A separate
//! history-only pass may match outputs against the already-generated public
//! address set so RECENT TRANSACTIONS can show RECEIVED activity wallet-wide.
//! No inactive UTXO is inserted into the spendable active cache.
...
/// Extend transaction history with incoming activity for every already
/// generated wallet address, without touching the active UTXO cache.
///
/// `generated_addresses` maps public owner bytes to deterministic key indices.
/// It contains public data only.  This pass runs after the ordinary active
/// scanner, so an active-address RECEIVED row may already exist.  In that case
/// its amount is upgraded to the complete wallet-wide amount instead of adding
/// a duplicate TXID.
///
/// Transactions already classified as SENT are never duplicated as RECEIVED.
/// This keeps one logical transaction row in the mobile recent-transaction
/// surface while preserving the existing active-address send semantics.
pub fn update_walletwide_received_history_from_block(
    history: &mut Vec<TxHistoryEntry>,
    generated_addresses: &HashMap<[u8; 32], u32>,
    block: &Block,
) -> Result<bool, String> {
    if generated_addresses.is_empty() {
        return Ok(false);
    }

    let stream = noid_chain::validate_block_page_stream(&block.transactions).map_err(|error| {
        format!(
            "wallet-wide history block h={} has invalid PagedSpend stream: {error}",
            block.header.height
        )
    })?;
    ...
    let block_tx_hashes: Vec<[u8; 32]> = noid_chain::try_compute_logical_txids(&block.transactions)
        .map_err(|error| {
            format!(
                "wallet-wide history block h={} has invalid txids: {error}",
                block.header.height
            )
        })?
        .into_iter()
    ...
    let mut record_received =
        |tx_hash: [u8; 32], is_coinbase: bool, outputs: Vec<&noid_tx::TxOutput>| {
            // A transaction we originated remains one SENT row even when one
            // of its outputs pays another generated address in this wallet.
            if history
                .iter()
                .any(|entry| entry.tx_hash == tx_hash && entry.direction == TxDirection::Sent)
            {
                return;
            }

            let mut amount = 0u64;
            let mut first_match: Option<(u32, noid_poseidon2b::primitives::Address)> = None;

            for output in outputs {
                let Some(&key_index) = generated_addresses.get(&output.owner.0) else {
                    continue;
                };
                ...
                if first_match.is_none() {
                    first_match = Some((key_index, output.owner));
                }
            }
            ...
            if let Some(existing) = history
                .iter_mut()
                .find(|entry| entry.tx_hash == tx_hash && entry.direction == TxDirection::Received)
            {
                // The active scanner may already have inserted a partial
                // active-address amount. Replace it with the complete amount
                // across every generated owner.
                if existing.amount_micronoid != amount
                    || existing.height != height
                    || existing.block_hash != Some(block_hash)
                {
                    existing.amount_micronoid = amount;
                    existing.height = height;
                    existing.block_hash = Some(block_hash);
                    existing.timestamp = timestamp;
                    existing.is_coinbase = is_coinbase;
                    changed = true;
                }
                return;
            }

            let Some((key_index, owner)) = first_match else {
                return;
            };
    ...
    // Primary coinbase plus optional development payout.
    for system_index in 0..stream.user_start_index {
        let tx = &block.transactions[system_index];
        let outputs = tx
            .body
            .live_outputs()
            .map(|(_, output)| output)
        ...
        record_received(block_tx_hashes[system_index], system_index == 0, outputs);
    }

    // Canonical user transactions. Aggregate every paged output belonging to
    // any generated address into one logical TXID row.
    for (group_index, group) in stream.groups.iter().enumerate() {
        let start = stream.user_body_start(usize::from(group.start_page));
        let end = stream.user_body_start(group.end_page_exclusive());

        let outputs = block.transactions[start..end]
            .iter()
            .flat_map(|page| page.body.live_outputs().map(|(_, output)| output))
            .collect::<Vec<_>>();

        record_received(group.spend.logical_txid.0, false, outputs);
    }
